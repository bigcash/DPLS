...
model_args = dict(n_layer=n_layer, n_head=n_head, n_embd=n_embd, block_size=block_size,
                  bias=bias, head_dim = head_dim, rank=rank, q_rank = q_rank, using_groupnorm=using_groupnorm, vocab_size=None, dropout=dropout, scale_attn_by_inverse_layer_idx=scale_attn_by_inverse_layer_idx,
                  dpls_epsilon=dpls_epsilon, dpls_top_k=dpls_top_k, dpls_ignore_index=dpls_ignore_index, label_smoothing=label_smoothing) # start with model_args from command line
...
if init_from == 'scratch':
    ...
    config.dpls_epsilon = dpls_epsilon
    config.dpls_top_k = dpls_top_k
    config.dpls_ignore_index = dpls_ignore_index
...
# Update swanlab run name and out_dir if not resuming
if init_from != 'resume':
    # Update swanlab run name
    swanlab_run_name = f"M_{model_type}_{int(param_count_m)}m_Opt_{optimizer_name}_LR_{learning_rate}_D_{dataset}_T_{total_tokens_B:.2f}B_time_{current_date}_jobid_{job_id}"
    ...
    try:
        ...
        swanlab_run_name = f"M_{model_type}_{int(param_count_m)}m_Opt_{optimizer_name}_LR_{learning_rate}_D_{dataset}_T_{total_tokens_B:.2f}B_time_{resume_job_id}"
...
if swanlab_log and master_process:
    ...
    swanlab.init(project=swanlab_project, name=swanlab_run_name, config=swanlab_config)
...
if change_criterion:
    print("changing to criterion2")
    raw_model.criterion = raw_model.criterion2
change_criterion_iters = 60050
while True:
    ...
    if iter_num >= change_criterion_iters and change_criterion:
        change_criterion = False
        raw_model.criterion = raw_model.criterion1
        print("changing to criterion1")
